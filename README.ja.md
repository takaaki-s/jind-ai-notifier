...
[jindaiko](https://github.com/takaaki-s/jindaiko)（`jin`）の通知プラグインです。
同時に、jindaiko のプラグイン機構の公式 example も兼ねています — マニフェスト 1
...
- **jin（jindaiko）** — [PR #63](https://github.com/takaaki-s/jindaiko/pull/63)
...
jindaiko はプラグインを、マニフェストの `name:` フィールドが示すディレクトリ名の下
にインストールします — **リポジトリ名ではなく `notifier` です**。この名前がそのまま
`jin plugin run` に渡す名前にもなります。したがって以下のコマンドはすべて
`notifier` を使います（例: `jin plugin run notifier`）。
...
- jindaiko の **内蔵** デスクトップ通知も発行されるため、各通知が二重に表示されます。
  内蔵の通知機能は将来削除予定であり、本プラグイン側では抑制しません。

## example として

このリポジトリは jindaiko のプラグイン機構のリファレンス example です。すべては
