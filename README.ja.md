...
- **jin（jind-ai）** — [PR #63](https://github.com/takaaki-s/jind-ai/pull/63)
  ...
  環境変数に依存します。
  [使い方](#使い方) で案内する主経路のショートカットには、加えて
  [PR #95](https://github.com/takaaki-s/jind-ai/pull/95) のプラグイン別
  keybinding 機能（merge SHA `9c7caa2`、それ以降のリリース）が必要です。旧
  tmux 経路は上記プラグイン拡張が入った jin であれば動作します。
- **bash 4+**
...
<details>
<summary>旧経路: tmux 設定のみで完結させる</summary>

jin がプラグイン別ショートカットバインディングに対応する前
（[必要要件](#必要要件) 参照）は、普段使いの（外側の）tmux 設定に直接キーバ
インドを書いて同じ動作を得ていました:
...
native なバインディング機能が入る前の jin を使っている場合や、キーマップを
自分の tmux 設定内で完結させたい場合はこちらを使ってください。
